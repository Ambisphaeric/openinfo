...
export const BlockTypeName = Type.Union(
  ['now', 'moments', 'relevant-now', 'ledger', 'pinned-doc', 'hint', 'ask', 'todos', 'drafts', 'custom'].map((b) => Type.Literal(b)),
  { $id: 'BlockTypeName' },
)
...
export const BlockQuery = Type.Object(
  {
    source: Type.Union(
      ['relevant-now', 'moments', 'ledger', 'sessions', 'pins', 'entities', 'todos', 'drafts'].map((s) => Type.Literal(s)),
    ),
