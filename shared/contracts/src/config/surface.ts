...
export const BlockTypeName = Type.Union(
  ['now', 'moments', 'relevant-now', 'ledger', 'pinned-doc', 'hint', 'ask', 'todos', 'custom'].map((b) => Type.Literal(b)),
  { $id: 'BlockTypeName' },
)
...
export const BlockQuery = Type.Object(
  {
    source: Type.Union(
      ['relevant-now', 'moments', 'ledger', 'sessions', 'pins', 'entities', 'todos'].map((s) => Type.Literal(s)),
    ),
