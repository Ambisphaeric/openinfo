...
│  ├─ screen/                   P4B  screen understanding (BUILT): processor.ts (subscribes capture.received, gated screen.ocr; source:'screen' image frames → invokeOcr → OcrResult + a distillate; skips the companion meta chunk; own last-failures ring) · router.ts (GET /screen/results · /screen/status) · registry.ts (WeakMap store→processor bridge, so the http.ts-mounted router reaches the processor's in-memory status) · index.ts (wireScreenOcr — main.ts wires it onto app.bus/app.store). The ocr/vlm INVOCATION lives in fabric/invoke.ts with the other slots; this is the processor+router only
