...
│  ├─ index/                    P2   extract (entities) · rank (recency×frequency) · relevant (relevant-now join) │ P4D (BUILT): canon.ts (mergeCanon — pure reference merging: same-kind duplicates whose name∪alias keys intersect fold into one canonical record via union-find, canonical winner = outboundCount→mentions→firstSeen→id; unions aliases/momentRefs/provenance, sums mentions+outboundCount, sets canonicalOf; relevant.ts folds through it before ranking) · rank.ts canon weight (× (1+outboundBoost·log2(1+outboundCount)) — "sent outranks viewed", log-damped, outboundCount 0 leaves score unchanged; write-side deferred — nothing increments outboundCount yet) · ingest/ (BUILT: chunk.ts page-anchored chunking [pure, deterministic ids] · fetcher.ts file[form-feed pages]/url[pageless]/pdf[honest stub]/gdoc[seam behind ingest.gdoc] · ingest.ts lifecycle → store pins/pin_chunks). Store: pins + pin_chunks tables (workspace-level canon, not session-keyed → not in moveSession)
