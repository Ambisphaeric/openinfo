...
test('GET /layouts/surfaces lists surfaces (seeded + user), and PUT emits surface.updated', async () => {
  ...
  try {
    ...
    // the list starts with the seeded default surfaces (the HUD + the #100 fields app)
    const initial = (await (await fetch(`${base}/layouts/surfaces`)).json()) as Surface[]
    assert.deepEqual(initial.map((s) => s.id).sort(), ['surf-openinfo-fields', 'surf-openinfo-hud'])
    ...
    assert.deepEqual(after.map((s) => s.id).sort(), ['surf-mine', 'surf-openinfo-fields', 'surf-openinfo-hud'])
  } finally {
    await app.close()
    await rm(dir, { recursive: true, force: true })
  }
