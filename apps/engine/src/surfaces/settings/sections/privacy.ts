 ...
 * Network) are macOS TCC grants owned by the CLIENT app; this authenticated engine-served page still
 * CANNOT detect grant state without new client→engine plumbing. Rather than fake a status, it says plainly
 * what each permission is for, where to grant
...
const GRANTS: readonly Grant[] = [
  ...
  {
    title: 'Local Network',
    what: 'Only for an explicitly configured HTTPS tunnel or other approved remote model/service. The engine itself refuses direct plaintext LAN binding.',
...
export const renderPrivacy = (_data: SetupData): string =>
  ...
  'the desktop app by macOS — this authenticated engine page can’t read their state, so it tells you what ' +
  'each is for and where to grant it. The menu-bar tray shows live status ' +
  ...
  'can see TCC). A browser page can’t detect it — so nothing here is invented. Workspace data is local ' +
  'by default; configured tunnels or model endpoints may move explicitly authorized data. Keys are ' +
  'write-only (never re-shown) under Keys.</div>'
