...
import type { Distillate, FieldValue, Moment, Pin, QueryResult, Surface, TodoItem } from '@openinfo/contracts'
...
/** Results parallel to the shipped stack: [pins, now(query-less), moments, center-summary, right-rolling, todos, fields]. */
const results = (): (QueryResult | undefined)[] => [
  q('pins', [pin]),
  undefined,
  q('moments', [noteMoment]),
  q('distillates', [summaryDistillate('dist-summary', 'AI summary — renewal terms agreed')]),
...
test('the shipped note-taker document declares the three-zone stack via id prefixes', async () => {
  ...
  assert.equal(zoneOf(surface.stack[1]!), 'center')
  assert.equal(zoneOf(surface.stack[4]!), 'right')
  assert.equal(zoneOf({ block: 'now' } as Surface['stack'][number]), 'center')
  const parts = partitionZones({ surface, now, results: results() })
  assert.deepEqual(parts.left.stack.map((b) => b.id), ['nt-left-pins'])
  assert.deepEqual(parts.center.stack.map((b) => b.id), ['nt-center-now', 'nt-center-notes', 'nt-center-summary'])
...
test('the shipped note-taker document renders three zones through the real renderer, blocks in the right column', async () => {
  ...
  // LEFT rail: home + feature nav + the honest Meetings/Archives folders + the Pins list (real pinned-doc)
  assert.match(left, /class="nt-home"/)
  assert.match(left, /Notes<\/button>/)
  assert.match(left, /Meetings</)
  assert.match(left, /Archives</)
  assert.match(left, /session-list block pending/) // the honest gap note, never a blank folder
  ...
  assert.doesNotMatch(center, /Rolling — Dana/) // the rolling summary belongs to the RIGHT sidebar

  // RIGHT sidebar (enrichments): the rolling summary + action items + fast fields
  assert.match(right, /Enrichments</)
  assert.match(right, /Rolling — Dana asked about SOC 2/) // the rolling-summary distillate stream
  ...
  assert.doesNotMatch(right, /AI summary — renewal terms agreed/) // the center summary stays in the center
})

test('the note-taker right-sidebar enrichments are honest when empty (fields on-match hidden, summary explained)', async () => {
  const surface = await loadNotetaker()
  const empty: (QueryResult | undefined)[] = [q('pins', []), undefined, q('moments', []), q('distillates', []), q('distillates', []), q('todos', []), q('fields', [])]
  const { center, right } = zones(renderNotetaker({ surface, now, results: empty }, defaultBlockRegistry))
  // always-visible summary/rolling blocks explain themselves rather than vanish (never a blank canvas)
  assert.match(center, /No distilled windows yet/)
