...
## Slice: chat context assembly READS the bundle declaration — assembly is data, not code  *(pill P1, branch feat/pill-p1-chat-context)*

PR #160 landed the Bundle substrate with a DECLARED `chat.contextAssembly` (an ordered list of the seven `ChatContextSource` kinds, each with optional `limit`/`windowChars`/`tokenBudget`), but the chat route still hard-coded WHAT entered the corpus. This slice makes the route READ the declaration and assemble accordingly — the owner canon that context assembly is DATA, not code, so a future Surface DSL compiles onto it.

### What landed
- **`engine/api/context-assembly.ts` (NEW, pure, unit-tested)** — `assembleChatContext(sources, gathered)` iterates the DECLARED sources IN ORDER, mapping each of the seven kinds to data the engine already holds, capped by that source's own declared budget: `bundle-prompt` (the app's priming), `active-preset` (the P2 seam), `transcript-window` (rolling char window, keeps the MOST RECENT chars), `insights` (distillate summaries), `relevant-entities` (recency×frequency join), `attached-docs` (the pin's cited page-anchored chunks — the #134 packing, now a declared source), `recent-turns` (request.history, ride as messages not system). Returns the composed system block, `historyTurns`, citations, and — the honest-accounting core — one `SourceReport` per source (`included`/`capped`/`empty`/`unavailable`) plus a `truncated` roll-up. `describeAssembly` renders the visible per-source note; `DEFAULT_CONTEXT_SOURCES` is the no-plan fallback.
- **`engine/api/chat.ts` (rewired)** — `runChat` no longer owns WHAT enters context; it gathers each declared source's data over a widened `ChatDeps` (`contextSources`, `bundlePrompt`, `transcript`, `insights`, `resolveActivePreset?` + the existing relevant/pin gatherers) and hands it to the pure assembler. `computeBudget` now takes an `assemblyNote` string and prepends the per-source disclosure to the turns estimate (extends the #134 visible-budget shape — same `ChatBudget.note` field, richer honest content). `assembleContext`/the pin-packing moved into the new module; `estimateTokens` moved there and is re-exported from chat.ts so existing importers are unbroken. `BUNDLE_PROMPT` is the shipped chat priming, delivered THROUGH the `bundle-prompt` source (drop that source and the app runs unprimed — a declaration choice, disclosed, not a code path).
- **`engine/api/http.ts` (chat-route region only)** — `postChat` resolves the seeded Standard App bundle (`ctx.bundles.get(DEFAULT_BUNDLE_ID)`), reads `bundle.chat?.sources ?? DEFAULT_CONTEXT_SOURCES`, and builds the gatherers (transcript = the process-global TranscriptRing joined oldest-last; insights = `listDistillates(ws).map(text)`; unknown-workspace guards preserved). The `active-preset` seam is left UNFILLED, degrading to `unavailable`.

### The preset read-seam (for P2 to fill)
`ActivePresetRef { label, text }` + optional `ChatDeps.resolveActivePreset?(workspaceId) => ActivePresetRef | undefined`. THREE honest states: seam ABSENT (deps omit it) ⇒ report `unavailable` (machinery not wired); seam present, returns `undefined` ⇒ report `empty` (wired, no preset selected); returns a ref ⇒ `included` (overlay injected as a `Voice/register — <label>:` block). P1 defines and consumes only this narrowest read; P2 owns preset documents/selection and fills the seam by supplying `resolveActivePreset` in the route's `ChatDeps` — a one-line wire, no assembler change.

### Ride-along
Added the bad-chat-source-kind HTTP twin to `http.test.ts`'s `GET/PUT /bundles` test: a PUT carrying `chat.sources: [{ kind: 'weather' }]` is rejected 400 by the SAME Tier-A gate (the closed `ChatContextSourceKind` union) that rejects a bad face kind — a source the assembler has no path to gather never persists.

### Tests + verification
`pnpm -r build && pnpm -r test` green before each commit. Engine **755** pass (serialized, CI-style; +14 over the prior count from the new `context-assembly.test.ts` [10] + the two new `chat.test.ts` declaration/omission cases + the http ride-along, net of the relocated assembleContext cases). Contracts **89**, client **457** (serialized; the full parallel `-r` run surfaced one client-suite timing flake that passes in isolation — the known #71-class parallel flake, unrelated to this engine-only slice). The heart of P1 is proven by `context-assembly.test.ts`'s "a stored bundle edit changes assembly with no code change": it `BundleDocuments.save`s an edited Standard App (chat plan trimmed to `bundle-prompt` only), re-reads it version-stamped, and observes the SAME assembler code produce a DIFFERENT context — and `chat.test.ts` proves it end-to-end over a live openai-compat server (a declaration omitting sources drops entities/excerpts from the real system prompt).

### Disclosed deviations / decisions
- **NO contract change.** Honest per-source accounting is structured internally (`SourceReport[]`, unit-tested) and surfaced to the user by enriching the existing `ChatBudget.note` — no new `ChatReply`/`ChatBudget` field, so the hot `shared/contracts` index/AllSchemas was left untouched (client rendering of structured per-source reports is out of scope this slice; the visible channel is the note).
- **`bundle-prompt` maps to the engine default** (`BUNDLE_PROMPT`), since Bundle carries no custom-prompt field yet. A future custom-prompt bundle field would feed the same source (additive, later) — flagged as an open DSL question already recorded in the substrate slice's notes.
- **`insights` = distillate summaries** (the clearest already-persisted "session insights"); moments/fields are a candidate to fold in later. `transcript-window` reads the process-global ring (not workspace-scoped — disclosed in the gatherer comment), matching how the diagnostics inspector already sources it.
## Slice: the five editable presets, ACTUALLY injected  *(pill P2, branch feat/pill-p2-presets)*

The named differentiator under "the pill": upstream glass shipped five editable context presets (School / Meetings / Sales / Recruiting / Customer Support) but their text was NEVER injected into the live prompt path — stored-and-editable but vestigial. This slice makes them actual documents, selectable per workspace, and ACTUALLY injected into the distill/insight pass. Owner canon honored: "defaults are just documents we ship" (presets ARE documents, edited over the existing document routes, no new editing UI); "prompts ship SIMPLE" (#130 — one neutral sentence each, no baked voice vector). Substrate reuse over invention throughout.

### Presets are prompt-template documents (`engine/presets/`, contract `config/promptTemplate.ts`)
- Additive-append `'preset'` to the `PromptTemplate.kind` closed union (mirrors the other closed-union grow-by-appending). A preset is therefore a `prompt-template`-kind document like distill/extract/field — so it seeds into the SAME `_meta.db` store and enumerates over GET /templates + edits over PUT /templates/:id with ZERO new route and no editing UI. No new doc kind, no new AllSchemas entry (the kind literal is the whole contract delta on the template side).
- `defaults.ts`: the glass-parity five as `builtin` preset docs (ids `preset-school`/`-meetings`/`-sales`/`-recruiting`/`-support`). Bodies are SIMPLE — a single "Context: this is a …; favor …" sentence, no persona, no `{{tone}}…{{voice.rules}}`, no `slot`. Not copied verbatim from glass — adapted to neutral factual framing.
- `documents.ts` `PresetDocuments`: `ensureDefaults` (seed-if-absent, never clobbers a user edit) · `list`/`get` kind-guarded (an ordinary template is never treated as a preset) · `isPreset` (the PUT-validity gate) · `activeId` (raw seam) · **`resolveActive(workspaceId)`** — the ONE "resolve the workspace's active preset if set" read the distiller AND the chat context-assembly path (P1) both consume, so injection and chat agree on a single source of truth. Degradable: unset ⇒ undefined; a dangling selection (preset since deleted) ⇒ undefined ⇒ no injection, never an error.

### Selection is per-workspace store state (`engine/store/workspaces.ts`)
Followed the egress idiom exactly (the closest existing per-workspace state): an `active_preset` column on the workspaces meta table (additive migration in place for an existing `_meta.db`) + `getActivePreset`/`setActivePreset`. Kept OFF the `Workspace` object/contract (its own column, not rehydrated through `fromRow`) — it is workspace STATE, not a workspace field, so the `Workspace` contract (`additionalProperties:false`) is unchanged. `getActivePreset` is the low-level read P1's degradable seam lands on.

### The routes (`engine/api/http.ts`, template/preset region — NEVER the chat region P1 owns)
`GET /active-preset?workspace=<id>` → `{workspaceId, presetId|null, presets:[5]}`; `PUT` sets it with honest validation — a nonexistent preset ⇒ 400, a non-string non-null `presetId` ⇒ 400, `null` clears (back to no injection). No client picker this slice (API-level selection only). Left OUT of the `/routes` manifest to avoid inventing a response-schema, consistent with the handled-but-unregistered guard control-plane routes (`/guard/policy`, `/guard-holds`).

### ACTUAL injection (`engine/distill/distiller.ts`) + the regression guard
At the one right interpolation site — where the distill SUMMARY prompt is composed — the distiller resolves the window's workspace active preset (`presets?.resolveActive`, an OPTIONAL dep so every existing Distiller construction is unchanged) and PREPENDS its body as leading context: `prompt = preset ? `${preset.body}\n\n${basePrompt}` : basePrompt`. Chose distiller-level prepend over a `{{preset}}` template placeholder for two reasons: (1) the byte-identical guarantee is then EXACT — no preset ⇒ `prompt === basePrompt`, today's behavior, with no dependence on a template-body/version-refresh dance; (2) the active preset applies to WHATEVER distill template is in force, including a user-authored one, not only a template that happens to carry the placeholder. Only the summary pass is steered; the strict-JSON moments/entities passes are left bare (a persona/context overlay would risk their output grammar). `Distillate.provenance.presetId` (additive-optional on the contract) names the preset that shaped a summary — an honest why-record that renders only in the System/Ledger register, absent (never fabricated) when no preset was active.

### The read seam for P1 (chat context assembly)
P1 (chat) consumes the `active-preset` chat context source through a narrow "resolve the workspace's active preset if set" read. That read is `PresetDocuments.resolveActive(workspaceId)` (resolved document) over the low-level `store.getActivePreset(workspaceId)` (id). Both are exposed on `ctx.presets`, so P1's degradable seam lands cleanly on it at merge — unset ⇒ the source is simply omitted.

### Tests + verification
Engine 747 → 755 (`--test-concurrency=1`, the CI serialize posture): store round-trip/clear/persist (+1), `PresetDocuments` seed/kind-guard/`resolveActive`/edit-not-clobbered (+4), distiller injection — UNSET is byte-identical + no `presetId`, FLIP changes the prompt (preset body prepended) + stamps `provenance.presetId`, per-workspace isolation (+2), and the `/active-preset` route e2e over the REAL engine — five listed, 400 on nonexistent/non-string, set/clear round-trip, per-workspace scoping, and a preset edited over /templates round-trips (+1). Contracts 90 (the seeded `promptTemplate.preset.json` validates), client 457 — all green. Verification is the two real drives: the route test spawns the compiled engine and exercises GET/PUT over HTTP; the injection test drives the real `Distiller.distillChunks` and observes the actual prompt string + the provenance stamp.
...
## Slice: the Ask face — screenshot-on-send, streamed reply, explain-my-screen, ask-history  *(branch feat/ask-face)*

The glass-parity Ask vertical: **Ask = screenshot-on-every-send + streamed reply; an EMPTY send = "explain my screen."** Chat becomes the app-scoped PERSISTENT thread per workspace — wiring the ask-history upstream glass left vestigial is the named differentiator. One vertical across contracts + engine + client + a driven e2e; substrate reuse over invention at every organ.

### Screenshot path (the disclosed choice: ocr/vlm READ, not image attachment)
The frame is folded in through the EXISTING screen-understanding path, not a new attachment kind: `postChat` runs the request's one frame through `invokeOcr` (paddle-serving blocks, or an openai-compat VLM filling the ocr slot — the same slot-filling the P4B processor rides), and the RECOGNIZED TEXT enters the declaration-driven assembly as the eighth declared source, `screen` (`context-assembly.ts`, four honest states: no frame ⇒ `empty`, unreadable ⇒ `unavailable`, blank ⇒ `empty`, text ⇒ `included`/`capped` under the declared budget — all folded into the visible #134 note). Chosen as the narrowest fit: it composes with everything already assembled (presets, attached docs, history) and needs no image-capable chat LLM.
- **Capture is main-process, per explicit send, consent-gated.** `shell.ts` factors `grabPrimaryDisplayImage` out of the cadence grab; `captureAskFrame` takes ONE Δ-gate-free still per send (the user asked about THIS moment — an unchanged screen is still the answer), only when the screen sense is enabled (`cfg.screenEnabled`, opt-in default OFF) and the OS grant is not refused. A refusal answers `{ok:false, reason}` — the send proceeds WITHOUT the frame and the reason paints beside the budget note. No capture path exists outside the send invoke (this channel is never polled; not ambient).
- **Privacy: the frame can never leave the machine.** The read runs under `resolveEgress({contentClass:'screen'})` — the most-specific-denial-wins layer DENIES screen egress outright, so the egress gate skips any egress-classified ocr/vlm endpoint BEFORE bytes leave (an http test proves an egress-only ocr slot yields `screen (unavailable)` with the frame bytes on no outbound request). Only the derived TEXT rides the chat hop, which runs the SAME `typed` consent + #63 guard chain as every #134 turn.

### Streamed reply (`chat.delta`, the #58 transcript.updated idiom exactly)
`InvokeOptions.onDelta` (fabric): with a consumer present the completions request asks for SSE (`stream:true`) and each content delta forwards as it arrives; the RESPONSE content-type decides, so an endpoint that ignores the flag degrades honestly to ONE final chunk — no fake typewriter. Without onDelta the request body is byte-for-byte the legacy shape. Streaming honesty: once any delta was emitted, a mid-stream failure SURFACES instead of silently falling through to the next endpoint (partial output was already painted). `postChat` publishes each chunk as an ephemeral `ChatDelta {turnId, seq, text, done}` bus event (ws-rebroadcast beside `transcript.updated`; terminal `done:true` in a finally, success or failure), keyed by the request's CLIENT-minted `turnId`. Event-fed render, query-fed truth: the client paints deltas for its own in-flight turn as a provisional streaming turn; the HTTP `ChatReply` is the authoritative record that replaces it. Never persisted.

### Empty send = explain-my-screen (a DOCUMENT, #130 posture)
`tpl-ask-default` ('ask' appended to the `PromptTemplate.kind` closed union) ships as a builtin: a short neutral question, seeded seed-if-absent, read FRESH per empty send over the existing GET /templates/:id and edited over PUT — never a string baked into the client. The client resolves the body and sends it as the turn's message (so the visible user turn, the persisted thread, and the engine-side record all agree on what was asked). Empty send with NO frame available (sense off / bridge absent / refused) is an honest visible no-op — "Nothing to ask…" text with the reason, no POST, never a silent swallow.

### Ask-history (found vestigial ⇒ built)
Verified: chat turns were NOT persisted anywhere — `runChat` read `request.history` (the client re-sent its own memory each turn) and the window opened amnesiac. Built: a `chat_turns` table per workspace DB (create-if-absent in the open path; autoincrement `seq` orders a same-millisecond exchange) + `appendChatTurn`/`listChatTurns`/`countChatTurns` in the store (savePin idioms — Tier-A validation, ensure-workspace, workspace-level like pins so no session-move involvement). `postChat` persists the exchange AFTER a successful turn; the `recent-turns` source now reads the persisted thread (request.history remains the fallback against an empty store, so a mid-conversation client is never amnesiac); GET /chat/history serves the oldest-first tail (cap 50) with `total`+`truncated` disclosed, and the chat window rehydrates it on open (`seedHistory` — a truncated tail is disclosed at the top of the log; a live session is never clobbered by a late fetch).

### Seed upgrades without clobbering (bundles)
The shipped Standard App plan now declares the eighth source (`{kind:'screen', tokenBudget:1000}`). `BundleDocuments.ensureDefaults` grew the #130 seed-or-refresh discipline: an install whose seeded bundle is provably UNEDITED (version 1 AND byte-identical to a `PREVIOUS_DEFAULT_BUNDLE_BODIES` entry) refreshes to the new shipped doc (v2, at most once); any user edit is untouched — a plan that deliberately omits `screen` keeps omitting it, and the turn's note says so.

### Tests + verification
Contracts 90 (3 new schemas: ChatScreenshot/ChatDelta/ChatHistory; ChatRequest/Bundle/PromptTemplate/ChatContextSource* regenerated — `pnpm gen` run, diff-clean). Engine 769 → 776 serialized (`--test-concurrency=1`): assembler screen states (+1), chat seams — screenText called once/absent-degrades/throwing-degrades, persisted-thread-beats-history + fallback (+2), fabric streaming — SSE order + accumulated text + measured usage, ignore-stream honest one-chunk, legacy byte-identical body, RST-mid-stream no-fall-through (+4, one net new file region), the http vertical — SSE deltas keyed to the client turnId with terminal done, data-URI OCR capture, screen(1) note, /chat/history round-trip, second-turn history from the store (+1), the egress fail-closed screenshot proof (+1), bundle refresh/no-clobber (+2), store thread round-trip/cap/durability (+1). Client 457 → 461 (empty-send honest no-op repainted from the old silent-ignore, one-frame-per-send + refusal disclosure, empty-send document question + failing-read visibility, delta paint/dedupe/foreign-drop/reconcile, seedHistory disclosure/no-clobber); interaction + register lints green (no new affordances; no machine ids painted — the budget note remains the disclosure surface). **Driven e2e** (`pnpm --filter @openinfo/client test:e2e:ask`, real Electron + real hud.html/preload, served window): real-typed send ships one frame + minted turnId, the provisional streamed turn paints BEFORE the deliberately-late HTTP reply and reconciles residue-free, an empty send posts the tpl-ask-default body, the 4-turn thread rehydrates across a real window destroy+reopen, and a refused-capture empty send paints "Nothing to ask…" and posts NOTHING. chat-input + attach e2es re-run green.
...
The two items disclosed-deferred when the #87 CI guard landed (their targets sat in a parallel agent's file territory then, free now). (1) **Guard as a contracts test:** `shared/contracts/src/schema-drift.test.ts` regenerates the schemas into a throwaway `mkdtemp` dir (NEVER the committed tree) and diffs against `schemas/` — file-set + byte-for-byte — failing with the offending names and a `run \`pnpm --filter @openinfo/contracts gen\`` instruction. Because it's a contracts test it rides `pnpm -r test`, gating every PR the same way the CI step does (which stays unchanged). The regen goes to a temp dir via a tiny seam added to `tools/schema-gen/gen.mjs`: `SCHEMA_OUT_DIR` (resolved absolute) overrides the default output dir; unset, gen writes the committed `schemas/` exactly as before, so the CI `git diff --exit-code` guard is untouched. Proven both ways — clean on the current tree, and a hand-injected `Ack.json` edit made the test fail naming `Ack.json (out of date)` (reverted). (2) **README enforcement note:** `shared/contracts/README.md` now states `schemas/` is generated, never hand-edited; run `pnpm --filter @openinfo/contracts gen` after any contract-source change; drift fails both CI and the local `pnpm -r test` suite. Both DoD leftovers satisfied ⇒ this PR **closes #87**.

## Slice: the pill — Standard App window assembly (Listen/Ask faces, docked panels)  *(branch feat/pill-window)*

The MVP centerpiece: ONE glass-parity window — a compact header RECTANGLE (Listen / Ask / Show-Hide / settings-on-hover) with a DOCKED PANEL beneath. Owner canon: an ASSEMBLY of existing organs, not new capability. No new block type, no contract change — the `hud` face kind ("the always-on glance face — the pill") + surface docs express it.

### The architecture (and the circularity it resolves)
- The pill is a new seeded surface `surf-openinfo-pill` registered as the Standard App bundle's `hud` face (the catalog + boot open faces by surfaceRef; the seed-integrity guard requires exactly one hud face, hud first — so the pill occupies the hud slot). Its own stack IS the Listen glance content (`now`/`relevant-now`/`moments`/`fields`, the curated HUD stack), because the pill IS the glance face — non-circular.
- The **Ask** face is RESOLVED FROM THE BUNDLE at open: GET /bundles → the `chat` face surfaceRef → GET /layouts/surfaces/:ref. The pill mounts whatever surface the chat face names (`surf-openinfo-chat` by seed: the `input` block + the streamed-reply/history organs), so **a different bundle produces a different Ask panel** — the data-driven requirement made real (and testable: `chatFaceRefForPill` + the e2e). The chat surface's stack is query-less (`now`+`input`), so mounting it needs no second hydration loop.
- **Forks nothing:** each face body is the SAME generic `renderSurface`. `createPillRenderer` (client `hud/pill-layout.ts`) is the header + panel frame, signature-compatible with `renderSurface` and selected by surface id in dev-entry — exactly the note-taker's precedent. The #58 live-transcript strip stays controller-composed (the Hud appends it to the frame); pill CSS parks it under the panel and hides it on the Ask face.

### Geometry — the three-state height authority
- The pill window has THREE heights, not the PanelController's two: BAR (Show-Hide off — the "dramatically shortened default HUD"), LISTEN (the glance panel), ASK (~3× the bar). `PillController` (client `hud/pill.ts`) is the window's ONE height authority (S1) — installed instead of PanelController/auto-resize for `surf-openinfo-pill`, over the SAME `hud:panel-size` bridge. Pure `pillHeight`/`pillExtentsFromPanel` + injected bridge, asserted headless. Selecting a face opens the panel to its extent; Show-Hide collapses to the bar, independent of face.
- The surface `panel` contract carries the two extents the shell needs (collapsed=bar 56, expanded=ask 432 ≈ the recorded chat geometry); the intermediate LISTEN extent (300) is a CLIENT layout constant (`PILL_LISTEN_EXTENT`) — disclosed, since the contract has no third field and a pill face kind is (rightly) not needed.

### Honesty
- Every header affordance is LIVE or honestly DISABLED (the interaction lint scans the served pill frame): Listen/Show-Hide/settings are wired verbs (`pill-face`/`pill-toggle`/`pill-settings` added to `WIRED_VERBS` + `wireActions`, the client-local mute-system-stream precedent — no writes, no paintFeedback). Ask is DISABLED-with-disclosure until its bundle face resolves (the note-taker Record posture); unresolved/absent ⇒ visible text in the Ask panel, never a blank.
- **Listen without a session** shows the honest state for free: the live-transcript strip returns null (no dead chrome) or "listening…" when live, and the glance blocks render a quiet dead heartbeat — the pill does NOT start capture (session control stays the tray's consent path; #136's in-window verb is still unbuilt, same posture as the note-taker Record button). The one capture the pill triggers is the Ask face's screenshot-on-send, still gated in main via the `openinfoScreen` bridge.
- **Settings-on-hover** opens the EXISTING settings path — a hover-revealed gear → `openinfoShell.openSettings` (new preload bridge) → `hud:open-settings` in shell.ts → the same `shell.openExternal(${engineUrl}/settings)` the tray opens. Not a new settings UI. (Disclosed: this added a small renderer→main bridge + ipc handler — a client-shell change, not an engine one; the honest alternative was a disabled affordance, but the existing path is real and cheap to reach.)

### Bundle refresh (#130)
The shipped `bundle.standard-app.json` hud face now points at `surf-openinfo-pill`; a second `PREVIOUS_DEFAULT_BUNDLE_BODIES` entry (the current hud→hud + `screen` body) lets an unedited v1 install refresh onto the pill. Disclosed limitation (pre-existing #130 semantics): an install already auto-refreshed to a v2 body is "settled" and won't re-refresh — fresh installs get the pill; some older auto-refreshed installs keep the old hud face until they edit/reinstall.

### Evidence
- Driven e2e `scripts/pill-e2e.mjs` (`test:e2e:pill`), real Electron + real preload: self-identifies (document.title `openinfo`); Ask resolves from /bundles; clicking Ask expands the REAL window to 432 and mounts the resolved chat input; a typed send flows end-to-end (one frame + client-minted turnId POSTed to /chat, streamed then reconciled); Show-Hide collapses the window to 56 and back; Listen restores the glance; settings fires the bridge. Observed LISTEN 300 / ASK 432 / BAR 56 — PASS.
- Unit: `hud/pill.test.ts` (height math + state machine), `hud/pill-layout.test.ts` (header + face bodies + honest states), `hud/pill-faces.test.ts` (bundle resolution + honest failures), + the interaction lint's served-pill scan. Engine 776 / client 482 unit tests green (the engine-link seam is the known #71 parallel timing flake — passes in isolation).

## Slice: the pill visibility fix wave — the pill IS the anchor  *(branch fix/pill-visibility)*

The owner live-QA'd 0.0.14 and could not find the pill. Three root causes (screenshots + measured bounds), fixed exactly; the fourth item is the policy lesson made a durable test.

### 1 · Anchor repoint (the discoverability bug)
`DEFAULTS.surfaceId` (`config.ts`) was `surf-openinfo-hud`. The ⌘\ accelerator + tray Show/Hide act on the anchor window `createHudWindow` opens from THAT default — so summoning the app revealed the OLD full hud, and the pill only ever opened as a buried Apps-folder window. Repoint the default to `surf-openinfo-pill`. Verified facts: nothing persists `surfaceId` (client.json/window-state/apps-state all leave it unset) ⇒ no migration; `OPENINFO_SURFACE` still overrides for dev. Composition holds because `createSurfaceWindow` resolves the pill's FULL spec (hud chrome + `focusable:true`) via `surfaceWindowSpec` — the anchor opens focusable at the HUD floor, the `PillController` becomes its sole height authority, and `showHud`/`hideHud` (showInactive/hide) only toggle window visibility, never the panel extents. `config.test.ts` now pins the resolved default so a silent repoint can't ship unnoticed again.

### 2 · The microsquare (the fill bug) — and the finding the e2e forced
`.pill-app` lacked the fill invariant every surface inherits from `.hud` (styles.ts:41), so it shrink-wrapped to intrinsic content and floated centered (measured: a 295×44 bar of naked buttons in a 708px window); `.pill-bar` had no background, so the collapsed bar read as floating buttons. Fix with the EXISTING idiom (no new visual language): `.pill-app{width:100%;max-width:660px;min-width:0}` + `.pill-bar` the `.hud` glass-card idiom (`--s-glass` fill, backdrop blur, `--s-line` border, 15px radius, floating shadow). **The anchor-path e2e then MEASURED that this was not enough** — `.pill-app` still rendered 293px. Root cause: `width:100%` resolves against the mount `<div>`, a flex item of the centered `.stage` that shrink-wraps to the pill's narrow content, so the percentage had nothing definite to fill (the full HUD only escapes this because its content is intrinsically ~660px wide; the pill's collapsed bar is not). Completed the fill by giving the mount a definite width: dev-entry tags the pill's mount `.pill-mount`, pill-styles sets `.pill-mount{width:100%}` (pinned to the stage's definite content box = window − 24px/side margin). Measured after: `.pill-app` = **660px at bar / listen / ask** (was 293). A pure-CSS-on-`.pill-app` fix cannot fill here; the intermediate mount had to be made definite-width.

### 3 · Reopen dedupe (disclosed: already implemented by the existing guard)
`reopenPersistedApps` reopens every `apps-state openApps` entry at boot; a QA-era entry still listing `surf-openinfo-pill` (the pill used to open as an Apps-folder window) would open a DUPLICATE second pill beside the now-pill anchor. The dedupe was ALREADY in place — `if (surfaceId === cfg.surfaceId) continue`, keyed to the exact id `createHudWindow` builds the anchor from — so repointing the default (fix 1) makes that entry skip automatically. No functional change was needed beyond fix 1; the comment now names the pill-anchor dedupe explicitly so the skip is not later mistaken for HUD-only bookkeeping. The reopen feature is intact (a left-open note-taker still returns); `openApps` is NOT cleared.

### 4 · The policy lesson made a test
The pill e2e drove the pill window DIRECTLY (`surfaceWindowSpec(PILL)` by hand) and so stayed green while the anchor gap shipped — it never entered through the user's door. POLICY (now in the script header): a driven e2e must enter through the user's ENTRY POINTS, not reach past them. Added SCENE 0, the anchor path: it resolves the shell config the way boot does (`resolveShellConfig({})` → shipped DEFAULTS), builds the anchor exactly as `createHudWindow` does from `cfg.surfaceId`, and asserts (a) the anchor default IS the pill and the PILL surface loads there (title + header rectangle), not the old hud; (b) `.pill-app` FILLS the window at bar/listen/ask (DOM rect ≥ window width − stage margin — the microsquare guard, forever); (c) the anchor show/hide path (hide()/showInactive()) shows/hides without disturbing the pill height (432 → 432). A `window-all-closed` no-op guard lets scene 0 destroy its window before scenes 1-6 build theirs without tripping Electron's auto-quit.
...
`hud/notetaker-layout.test.ts` asserts the new Record note and `doesNotMatch(/#136/)`. Gates: contracts 110,
fixtures 15, client 588 (+1), engine 1035 — no contract or engine change (pure client copy + tests).
